...
def create_coordinates(
    xds, MeasurementSet, baseline_ant1_id, baseline_ant2_id
):
    ...
    delay_direction = {'data':list(field.get('DELAY_DIR', [])), 'attrs': {'units': 'rad', 'type':'sky_coord', 'description':'Direction of delay center in right ascension and declination.'}}
    
    phase_direction = {'data':list(field.get('PHASE_DIR', [])), 'attrs': {'units': 'rad', 'type':'sky_coord', 'description':'Direction of phase center in right ascension and declination.'}}
    reference_direction = {'data':list(field.get('REFERENCE_DIR', [])), 'attrs': {'units': 'rad', 'type':'sky_coord', 'description':'Direction of reference direction in right ascension and declination. Used in single-dish to record the associated reference direction if position-switching has already been applied. For interferometric data, this is the original correlated field center, and may equal delay_direction or phase_direction.'}}
    name = field['NAME']
    code = field['CODE']
    ...
    xds.frequency.attrs['reference_frequency'] = MeasurementSet.SPECTRAL_WINDOW[0].get('REF_FREQUENCY', '')
    xds.frequency.attrs['effective_channel_width'] = MeasurementSet.SPECTRAL_WINDOW[0].get('EFFECTIVE_BW', '')[0]

    channel_widths = MeasurementSet.SPECTRAL_WINDOW[0].get('CHAN_WIDTH', '')
    
    if not isinstance(channel_widths, str):
        unique_chan_width = np.unique(channel_widths[np.logical_not(np.isnan(channel_widths))])
        xds.frequency.attrs['channel_width'] = {'data':np.abs(unique_chan_width[0]), 'attrs':{'type':'quanta','units':'Hz'}}
...
def get_baselines(MeasurementSet: tables.table) -> np.ndarray:
    ...
    ant1, ant2 = MeasurementSet.getcol('ANTENNA1', 0, -1), MeasurementSet.getcol('ANTENNA2', 0, -1)
    baselines = np.array(
        [
...
def create_attribute_metadata(column_name, MeasurementSet):
    ...
    if column_name in ['U', 'V', 'W']:
        ...
        attrs_metadata['units'] = column_description.get('keywords', {}).get('QuantumUnits', ['None'])[0]
...
def add_time(xds, MeasurementSet):
    ...
    xds.time.attrs['units'] = time_description.get('keywords', {}).get('QuantumUnits', ['None'])[0]
        
    xds.time.attrs['time_scale'] = time_description.get('keywords', {}).get('MEASINFO', {}).get('Ref', 'None')
...
def MS_chunk_to_zarr(xds, MeasurementSet_chunk, time, time_indices, baseline_ant1_id, baseline_ant2_id, column_names, outfile, append):
...
def convert(infile, outfile, compress=True):
    ...
    with tables.table(infile) as MeasurementSet:
        ...
        time_indices = range(len(unique_time_values))#np.searchsorted(unique_time_values, MeasurementSet.getcol('TIME'))
        ...
        xds_base = create_coordinates(xds_base, MeasurementSet, baseline_ant1_id, baseline_ant2_id)
        ...
        for i, time in enumerate(unique_time_values[1:]):
            print(i+1)
            MS_chunk_to_zarr(xds_base.copy(deep=True), MeasurementSet.query('TIME == $time'), time, time_indices, baseline_ant1_id, baseline_ant2_id, column_names, outfile_tmp, append=True)
